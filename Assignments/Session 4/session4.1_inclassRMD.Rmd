---
title: "STA363 - Class 07"
author: "Hughes, O'Connell, Wang"
date: "Sections A,C: Wednesday, September 18, 2019 <br> Section B: Tuesday, September 17, 2019"
...
* Build an appreciation for the difficulties that arise with advanced designs
* Recognize and analzye data from a repeated measures experiment
...
More elaborate designs can incorporate different aspects of simpler designs.  For example, we could have a Two-Way ANOVA with a Blocking factor (see Homework \#3). How to code this for analysis in R?
...
* **What are the experimental units?**    *Chunk of Coal* 
* **What are thre treatments in this design?**    *Any combination of type of coal, particle size, and weight.* 
* **How many unique treatments are in this design?**    *64. 4 levels of coal with 4 particle sizes with 4 different weights. 4x4x4=64.* 
* **Discuss the difficulties associated with replication in this design.**    *It would mean adding another observation for every treament. 64 observations costs time and money.* 
* **What challenges does having three factors introduce to the analysis of such a designed experiment?**    *It increases the cost and time to replicate the experiment* 
...
While the above looks fine, sorting the data by `Subject` emphasizes the fact that `Time` is a within-subjects factor:
...
**Why is this important to distinguish from a between-subjects factor?**  *ANSWER HERE*  
...
**What appears to going on, based on the plots?**  *ANSWER HERE*
...
Now let's proceed to an analysis.  First, we must articulate the **study goals** so we know how to address them via fitting a model to the data. 

First, recognize that the effect of the different diets and exercise regimens **would only be revealed over time.**  This means that we should compare how different treatments (i.e. different diet/exercise combinations) would affect mean pulse rates differently over time. Stated another way, we ask: **Does the effect of different tretments on mean pulse rate change depending on the time of measurement?**  That is what an **interaction test** between `Time` and the treatments (`Diet` and `Exercise` combinations) would address!
...
* The `Error(Subject/Time)` piece tells R that `Time` is a **within-subjects (repeated) factor**, and thus will handle the random error components appropriately for analysis.
...
The first thing to look at is the **highest order interaction** (labeled `Diet:Exercise:Time`). This test essentially addresses if the Diet/Exercise combinations affect pulse rate differently as time goes along (i.e. as the activity becomes more strenuous).  We see that this effect is not significant ($F$-stat = 1.850 on 4 and 24 degrees of freedom, $p$-value = 0.1523).  Because of this, we can look at lower-order interactive effects to see where (or *if*) there is evidence of differences in pulse rates:

* `Exercise:Time`: The effect of different exercise regimens (regardless of diet!) does **not** significantly affect pulse rates  as time goes along (i.e. as the activity becomes more strenuous) ($F$-stat = 0.320 on 4 and 24 degrees of freedom, $p$-value = 0.8740).
...
So, the only meaningful effect we have dicovered here is due to `Diet`.  The different diets significantly affect the true mean pulse rate, regardless of exercise regiment of type of physical activity.

You might ask why we don't perform any multiple comparisons here for the `Diet` effect on pulse rates.  **There are only two diets, there are not multiple diets to compare.**

#### Assumption checking

Due to the lack of independence in our responses (the within factor structure), checking the underlying assumptions (Normality, constant variance, independence between factors) is a complex endeavor and is outside the scope of this course. Contact a statistician if needed.
...
**Note about the plot** We are making a plot based on a different dataset we created! This new dataset consist of the Diet type, Time and mean and standard deviation of pulse rates. Below is the data we just plotted (this is only included for explanation).
...
*The effect of the diet does not seem to be influenenced by the type of actibity. Whatever the diet effect is, the differences are always about the same.*