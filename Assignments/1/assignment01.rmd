---
title: "Assignment 01"
author: "Beuerlein, Danger, Greco"
date: "Sections A,C: Monday, August 26, 2019 <br> Section B: Tuesday, August 27, 2019"
...
Next, let's see how *Israel* and *Iran (Islamic Repubilc of)*'s voting record changed over the years on each of the issues. Both of these countries were founded in the 20th Century and have a long history of political, militarily, and religious opposition to each other.

```{r plot-yearly-yes-issue, fig.width=10, warning=FALSE, fig.cap="Figure 2: Plots of UN voting records for the Israel and Iran on the following 6 issues over time: arms control, colonialism, economic development, human rights, nuclear weapons, and Palestinian conflict. The lines on the plot show that Iran (Islamic Repubilc of) consistently votes yes at a much higher rate than Israel. Iran (Islamic Repubilc of) has not voted on an Economic Development issue in the 21st century."}
un.compare <- un_votes %>%
  filter(country %in% c("Israel", "Iran (Islamic Republic of)")) %>%
  inner_join(un_roll_calls, by = "rcid") %>%
  inner_join(un_roll_call_issues, by = "rcid") %>%
  group_by(country, Year = year(date), issue) %>%
  summarize(votes = n(),
            percent_yes = mean(vote == "yes")) %>%
  filter(votes > 10)    # Only use records where there are more than 10 votes
...
**We chose Iran (Islamic Repubilc of) and Israel as our two countries to compare. Both of these countries inhabit a similar region of the planet but have a long history of military, ideology, political, and religious differences. They both are also newly formed countries, having both been created in the mid 20th Century.**
...
**The long history of opposition between Israel and Iran is reflected in their voting records. It is not suprising that both nations have viritually opposite voting records in the UN.**
